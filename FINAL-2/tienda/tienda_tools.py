...
class TiendaTools:
    ...
    def _reponer_tienda_desde_alamcen(cls, min_stock):
        ...
        try:
            ...
            my_params = {}
            my_params["available"] = True

            my_headers = {}
            my_headers["Content-Type"] = "application/json"
            my_headers["Api-Key"] = cls.API_KEY_ALMACEN
            ...
            if response.status_code == 200:
                ...
                if data:
                    for articulo_externo in data:
                        articulo = {}
                        articulo["codigo"] = articulo_externo["id"]
                        articulo["descripcion"] = articulo_externo["description"]
                        articulo["stock"] = articulo_externo["quantity"]
                        articulo["activo"] = articulo_externo["available"]
    ...
    def _check_producto_necesita_reponer(cls, articulo, min_stock):
        ...
        try:
            ...
            if reponer > 0:
                cls.reponer_producto_almacen(articulo, reponer, True)
        except Exception as e:
    ...
    @classmethod
    def reponer_producto_almacen_por_codigo(
        cls, codigo, cantidad, adapta_a_disponibilidad
    ):
        ...
        try:
            ...
            results = DatabaseTools().select_from_table(table, parametros)
            if results:
                print(results)
                print(results[0])
    ...
    @classmethod
    def reponer_producto_almacen(cls, articulo, cantidad, adapta_a_disponibilidad):
        """
        ...
        try:
            ...
            if articulo["stock"] < int(cantidad) and adapta_a_disponibilidad:
                cantidad = articulo["stock"]

            if int(cantidad) > 0:
                # Hacemos la petición de almacen adaptada al stock disponible
                my_body = {}
                my_body["decrease_quantity"] = int(cantidad)
                api_retira_alamcen = cls.API_ALMACEN_COMPRAR.replace(
                    ":product_id", str(articulo["codigo"])
                )
                print(api_retira_alamcen)
                ...
                if response_comprar.status_code == 200:
                    return cls.actualiza_entrada_stock(
                        articulo, articulo["codigo"], cantidad
                    )
                raise ValueError(
